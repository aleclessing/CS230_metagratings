...
def predict_spot_plot(lr_fields, hr_fields, pt_coos, pt_vals, sr_vals, colorbar=False, ax=None):
    import matplotlib.pyplot as plt
    if ax is None:
        fig, ax = plt.subplot(1,1)
    ax.imshow(hr_fields[0])
    ...
    plt.scatter(x=zs, y=xs, c=color_func(sr_vals[:,0]), s=100, edgecolors='black')
...
if __name__ == '__main__':
    ...
    if args.viz:
        predict_spot_plot(lr_fields, hr_fields, pt_coos, pt_vals.detach().numpy()[0], sr_pt_fields.detach().numpy()[0])
